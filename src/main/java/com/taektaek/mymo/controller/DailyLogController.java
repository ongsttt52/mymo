...
public class DailyLogController {
    ...
    public ResponseEntity<DailyLogResponse> createDailyLog(
            ...
            @Valid @RequestBody DailyLogCreateRequest request) {
        DailyLogResponse response = dailyLogService.createDailyLog(memberId, request);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @GetMapping("/{id}")
    public ResponseEntity<DailyLogResponse> getDailyLog(@PathVariable Long id) {
        DailyLogResponse response = dailyLogService.getDailyLog(id);
        return ResponseEntity.ok(response);
    }

    @GetMapping
    public ResponseEntity<List<DailyLogResponse>> getDailyLogsByMember(@RequestParam Long memberId) {
    ...
    public ResponseEntity<DailyLogResponse> updateDailyLog(
        ...
        DailyLogResponse response = dailyLogService.updateDailyLog(id, request);
        return ResponseEntity.ok(response);
    }
